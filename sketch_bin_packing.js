...
function packImages(imageDataList) {
  ...
  imageDataList.sort((a, b) => a.dominantColor.localeCompare(b.dominantColor));
  ...
  let packedImages = imageDataList.filter(imgData => imgData.fit).map(imgData => {
    return {
      ...imgData,
...
function createComposition(packedImages) {
  ...
  let maxX = Math.max(...packedImages.map(img => img.assignedX + img.assignedWidth));
  let maxY = Math.max(...packedImages.map(img => img.assignedY + img.assignedHeight));
